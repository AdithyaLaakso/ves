...
path = kagglehub.dataset_download("miswindall/al-pub-v2") + "\\ALPUB_v2\\images"
number_of_images_per_letter = 1
print(f"Dataset downloaded to {path}")
def reduce_contrast(image, image_age = 5):
    
def generate_irregular_blobs(size, smooth_sigma=8, min_area=300, max_area= 15000):
...
def generate_papyrus_fiber_texture(size, fiber_density=0.3, fiber_strength=1.5):
    ...
    angle2 = np.random.uniform(150, 210)
...
def add_ct_fragment_noise(gt_mask, fiber_density = .3, fiber_strength=0.4, blob_strength=0.3, gauss_sigma=1.0, output_size=(128, 128)):
    ...
    noisy = gt_mask + np.random.normal(0, gauss_sigma * 5, output_size) - blobs
...
gauss_sigma_values = [.001, .01, .05, .1, 0.3, 0.5, 4]
...
for letter in unique_greek_letters:
    ...
    for _ in range(number_of_images_per_letter):
        ...
        output_filename = f"{os.path.basename(img_path).split('.')[0]}_{letter}_blob_strength_{blob_strength}_gauss_sigma_{gauss_sigma}_{i}.png"
