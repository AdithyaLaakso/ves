...
class greek_char_prob_field:
    ...
    def __init__(self, probabilities: Optional[Union[List[float], np.ndarray, Dict[str, float]]] = None, start_char = None):
        if start_char is not None and start_char in GREEK_LETTERS:
            self.probs = np.zeros(48)
            idx = GREEK_LETTERS.index(start_char)
            self.probs[idx] = 1
        elif start_char is not None and start_char not in GREEK_LETTERS:
    ...
    def get_top_k(self, k: int = 5) -> List[tuple]:
        ...
        return [(GREEK_LETTERS[i], self.probs[i]) for i in sorted_indices[:k]]
    ...
    def to_dict(self) -> Dict[str, float]:
        ...
        return {char: prob for char, prob in zip(GREEK_LETTERS, self.probs)}
    def sample(self, n_samples: int = 1) -> Union[str, List[str]]:
        samples = np.random.choice(GREEK_LETTERS, size=n_samples, p=self.probs)
    ...
    def get_char_idx(char: str) -> int:
        ...
        if char not in GREEK_LETTERS:
            ...
            return GREEK_LETTERS.index(char)
    @staticmethod
    def get_char_from_idx(idx: int) -> str:
        return GREEK_LETTERS[idx]
    def __str__(self) -> str:
        top_5 = self.get_top_k(5)
        top_str = ", ".join([f"{char}: {prob:.3f}" for char, prob in top_5])
        return f"GreekCharProbability(top 5: {top_str})"
