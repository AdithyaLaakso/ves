...
# kernel path: /tmp/torchinductor_Adithya/he/chepng2jduqxtqprwfgc4px4g6bjj4q5orfwutsc64s4j63it6ly.py
...
#   %expand : [num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%neg, [9, 1]), kwargs = {})
#   %div_1 : [num_users=2] = call_function[target=torch.ops.aten.div.Scalar](args = (%expand, 9), kwargs = {})
...
#   %expand_1 : [num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_1, [9, 1, 32, 32]), kwargs = {})
...
#   %expand_2 : [num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_3, [9, 1, 32, 32]), kwargs = {})
...
def call(args):
    ...
    assert_size_stride(sigmoid, (9, 1, 32, 32), (1024, 1024, 32, 1))
    assert_size_stride(add_2, (9, 1), (1, 1))
    assert_size_stride(div_3, (9, 1), (1, 1))
    assert_size_stride(tangents_1, (9, 1, 32, 32), (1024, 1024, 32, 1))
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_add_div_expand_mul_neg_sigmoid_backward_unsqueeze_0.run(tangents_1, tangents_2, div_3, add_2, primals_1, sigmoid, buf0, 9216, stream=stream0)
        del add_2
        del div_3
        del primals_1
        del sigmoid
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    sigmoid = rand_strided((9, 1, 32, 32), (1024, 1024, 32, 1), device='cuda:0', dtype=torch.float32)
    add_2 = rand_strided((9, 1), (1, 1), device='cuda:0', dtype=torch.float32)
    div_3 = rand_strided((9, 1), (1, 1), device='cuda:0', dtype=torch.float32)
    tangents_1 = rand_strided((9, 1, 32, 32), (1024, 1024, 32, 1), device='cuda:0', dtype=torch.float32)
