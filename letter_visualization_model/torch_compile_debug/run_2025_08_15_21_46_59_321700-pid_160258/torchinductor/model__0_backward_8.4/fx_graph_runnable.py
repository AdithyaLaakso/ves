...
class Repro(torch.nn.Module):
    ...
    def forward(self, primals_1, sigmoid, add_2, div_3, tangents_1, tangents_2):
        ...
        expand = torch.ops.aten.expand.default(neg, [20, 1]);  neg = None
        div_1 = torch.ops.aten.div.Scalar(expand, 20);  expand = None
        ...
        expand_1 = torch.ops.aten.expand.default(unsqueeze_1, [20, 1, 32, 32]);  unsqueeze_1 = None
        ...
        expand_2 = torch.ops.aten.expand.default(unsqueeze_3, [20, 1, 32, 32]);  unsqueeze_3 = None
...
def load_args(reader):
    ...
    reader.tensor(buf1, (20, 1, 32, 32), is_leaf=True)  # sigmoid
    buf2 = reader.storage(None, 80, device=device(type='cuda', index=0))
    reader.tensor(buf2, (20, 1), is_leaf=True)  # add_2
    buf3 = reader.storage(None, 80, device=device(type='cuda', index=0))
    reader.tensor(buf3, (20, 1), is_leaf=True)  # div_3
    buf4 = reader.storage(None, 81920, device=device(type='cuda', index=0))
    reader.tensor(buf4, (20, 1, 32, 32), is_leaf=True)  # tangents_1
