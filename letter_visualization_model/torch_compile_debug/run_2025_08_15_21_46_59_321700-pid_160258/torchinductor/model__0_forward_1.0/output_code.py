...
# kernel path: /tmp/torchinductor_Adithya/ee/cee2o7goe4d4zrq2a5sigmzrg7wmq3yrneeqw4antofmmovfl7ty.py
...
# kernel path: /tmp/torchinductor_Adithya/sf/csfkbmjausprghogw4gkhth73hmyvsuzdcoyacoo43almqzsuohn.py
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        buf1 = empty_strided_cuda((20, 1), (1, 20), torch.float32)
        buf2 = empty_strided_cuda((20, 1), (1, 20), torch.float32)
        buf4 = reinterpret_tensor(buf2, (20, 1), (1, 1), 0); del buf2  # reuse
        buf6 = empty_strided_cuda((20, 1), (1, 1), torch.float32)
        # Topologically Sorted Source Nodes: [pred_probs, mul, intersection, sum_2, sum_3, union, mul_1, add_1, add_2, dice], Original ATen: [aten.sigmoid, aten.mul, aten.sum, aten.add, aten.div]
        stream0 = get_raw_stream(0)
        triton_per_fused_add_div_mul_sigmoid_sum_0.run(buf4, primals_2, primals_1, buf0, buf1, buf6, 20, 1024, stream=stream0)
        del primals_2
        buf5 = empty_strided_cuda((), (), torch.float32)
        buf7 = buf5; del buf5  # reuse
        # Topologically Sorted Source Nodes: [mul_1, add_1, dice, mean, dice_val], Original ATen: [aten.mul, aten.add, aten.div, aten.mean, aten.rsub]
        stream0 = get_raw_stream(0)
        triton_per_fused_add_div_mean_mul_rsub_1.run(buf7, buf1, buf4, 1, 20, stream=stream0)
        del buf1
